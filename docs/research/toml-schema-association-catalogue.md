...
> **Companion note.** `toml-schema-association.md` on this branch answers the same
> five questions with taplo, tombi and their language servers actually running.
> Where a cell here is marked `unverified` for want of a live editor, check there
> first; §7.4 of that note lists the three places the two disagree. This file is
> kept for its documentation- and source-level coverage of consumers the other did
> not reach — JetBrains' `.idea/jsonSchemas.xml`, Emacs `eglot`, `schemars` 0.8
> versus 1.x, and the `jsonschema` 0.17.1 per-draft keyword table.
